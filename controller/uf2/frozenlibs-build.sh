...
pip3 install --upgrade --force -r requirements-dev.txt --break-system-packages
pip3 install --upgrade --force -r requirements-doc.txt --break-system-packages
...
git submodule add https://github.com/adafruit/Adafruit_CircuitPython_ConnectionManager/
...
cd Adafruit_CircuitPython_AdafruitIO && git checkout 5.8.2 && cd ..
cd Adafruit_CircuitPython_ConnectionManager && git checkout 1.0.1 && cd ..
cd Adafruit_CircuitPython_MiniMQTT && git checkout 7.6.3 && cd ..
cd Adafruit_CircuitPython_RSA && git checkout 1.2.21 && cd ..
cd Adafruit_CircuitPython_Wiznet5k && git checkout 5.0.9 && cd ..
cd Adafruit_CircuitPython_asyncio && git checkout 1.3.1 && cd ..
cd Adafruit_CircuitPython_binascii && git checkout 2.0.3 && cd ..
cd Adafruit_CircuitPython_Logging && git checkout 5.2.5 && cd ..
cd Adafruit_CircuitPython_Requests && git checkout 3.2.3 && cd ..
cd Adafruit_CircuitPython_Ticks && git checkout 1.0.13 && cd ..
cd Adafruit_CircuitPython_WSGI && git checkout 2.0.2 && cd ..
cd Adafruit_CircuitPython_HTTPServer && git checkout 4.5.7 && cd ..
cd Adafruit_CircuitPython_datetime && git checkout 1.2.7 && cd ..
cd Adafruit_CircuitPython_IterTools && git checkout 2.1.0 && cd ..
