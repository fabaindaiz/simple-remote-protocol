...
def metrics(client: Handler, command: bytes, args: list[bytes]):
    gc.collect()
...
def reboot(client: Handler, command: bytes, args: list[bytes]):
    print("reboot command received, rebooting device...")
    client.send(b"Rebooting...")
    supervisor.reload()

@router.register(b"exit")
def exit(client: Handler, command: bytes, args: list[bytes]):
